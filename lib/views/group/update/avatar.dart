...
class UpdateGroupAvatarPage extends StatefulWidget {
  const UpdateGroupAvatarPage({Key? key, required this.notifyParent}) : super(key: key);

  final Function() notifyParent;

  @override
  State<UpdateGroupAvatarPage> createState() => _UpdateGroupAvatarPageState();
}

class _UpdateGroupAvatarPageState extends State<UpdateGroupAvatarPage> {
  Future getImgFromSrc(ImageSource imageSource) async {
    final pickedFile = await ImagePicker().pickImage(source: imageSource);
    if (pickedFile != null) {
      setState(() {
        pendingGroupAvatar = File(pickedFile.path);
      });
    }
  }
